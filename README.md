...
- **「移す」の作法**は生成物の `docs/ticket.md` に出る（連携を有効にした場合のみ）。
  候補を問い → 宣言した名前と突き合わせ → **移した後に読み直して確かめる**の三段。
  名前でそのまま動かせる環境（ラベル等）を基準に据え、**IDの解決が要る環境**
  （トランジション・単一選択フィールド等）ではその一段が挟まる、という抽象度で書いてある。
  宣言した状態名が候補に無ければ**設定の誤りとして報告して止める**（黙って続行しない）

**ウィザードは連携する / しないのどちらでも `ticket:` 節を書き出します。**
枠さえあれば、後から有効にするときにキーの綴りも「`status_*` を入れ子にしない」制約も一緒に目に入ります
（間違えても**エラーにならず黙って空を返す**ため、自力で書き足す形では気づけません）。

**状態名は組織で共有できます。** 対応表は組織の事実であってプロジェクトの事実ではなく、
同じワークフローを使う限り全プロジェクト共通です。

```yaml
# ~/.koumei/ticket-status-map.yaml （置いておけばウィザードが自動で読む）
...
- `setup.sh --ticket-map <path>` で明示もできます。**指定したのに読めなければ止まります**（黙って空で進めません）
- **マップは「config を書くための材料」であって、生成時に読むものではありません。**
  値は config に書き込まれ、`git diff` に出ます。生成時に読む方式だと、
  マップを持たない人が同じリポジトリで `setup.sh` を叩いたとき違う生成物ができてしまいます
- **`queue` はマップから取り込みません。** 行列条件はプロジェクト固有で、
  「自分の担当に絞る」ことが安全性の根拠です。共有ファイルに置けば絞り込みの共有＝事故になります
